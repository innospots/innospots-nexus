...
`java:dependency-upgrade` 专用：JDK、开发框架、构建插件与第三方依赖大版本迁移。
按升级类型组织；每类给出断点、执行顺序、验证要点、常见坑。

工程**自身产物版本号**（`${revision}`）见 `java:project-upgrade` → `project-version.md`。
...
5. 在 BOM 升级 Jackson / Hibernate 等连带组件
6. 替换不兼容的第三方库并扫描业务代码影响
7. 升到目标版本线的最新补丁版
8. 全量测试 + 回归
```
...
- Jackson 3 迁移（Jackson 2 以废弃形式提供）
- Hibernate 7、Tomcat 11
- 若干配置属性重命名
...
- 断言统一 AssertJ
- 详见 `java:develop` → `test-conventions.md`
