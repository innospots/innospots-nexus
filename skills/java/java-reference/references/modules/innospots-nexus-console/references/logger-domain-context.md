...
单次被拦截调用的框架无关描述。 {@link com.innospots.nexus.console.logger.InvocationLogHandler} 用于持久化。 不含框架类型，任意 Java 运行时均可产生并消费。本记录是从拦截器边界传入 审计日志操作器的标准领域上下文。</p>
...
| `action` | `String` | 由 {@link com.innospots.nexus.console.logger.AuditLog} 注解声明的业务动作编码 |
