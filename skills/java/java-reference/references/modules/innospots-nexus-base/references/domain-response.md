...
- **说明：** 紧凑构造器校验分页参数并确保记录列表永不为 null。 / public PageResult { if (pageNo < 1) { throw new IllegalArgumentException("pageNo must be greater than 0"); } if (pageSize < 1) { throw new IllegalArgumentException("pageSize must be greater than 0"); } if (total < 0) { throw new IllegalArgumentException("total must not be negative"); } records = records == null ? List.of() : List.copyOf(records); } /** 创建自动计算总页数的分页结果。
- **参数：**
  - `records` — 记录列表
  - `pageNo` — 从 1 开始的页码
  - `pageSize` — 每页记录数
