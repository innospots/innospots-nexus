...
平台基础运行时异常。携带机器可读的错误码（见 {@link StatusCode}）、 人类可读的消息以及可选的国际化展示信息供前端渲染。 {@link #build(String, String)}）而非直接调用构造函数。</p>

### 方法

#### `build(StatusCode statusCode) → NexusException`

- **说明：** 基于 {@link StatusCode} 构建异常，使用其完整码与摘要消息。
- **参数：**
  - `statusCode` — 状态码
- **返回：** 异常实例
...
- **说明：** 基于 {@link StatusCode} 构建异常，附带国际化展示消息。
- **参数：**
  - `statusCode` — 状态码
  - `display` — 国际化展示信息
- **返回：** 异常实例
...
- **说明：** 基于 {@link StatusCode} 构建异常，附带展示消息与原始原因。
- **参数：**
  - `statusCode` — 状态码
  - `display` — 国际化展示信息
  - `cause` — 原始异常
...
- **说明：** 基于 {@link StatusCode} 构建异常，使用覆盖消息。
- **参数：**
...
- **说明：** 基于 {@link StatusCode} 构建异常，附带原始原因。
- **参数：**
