...
### 5.2 ResolvedPluginDefinition

概念模型如下，具体 record 名称可在实现时保持现有兼容：

```java
public record ResolvedPluginDefinition(
        String pluginId,
        String version,
        int apiVersion,
...
public record CapabilityContribution<T extends CapabilityProvider>(
        ...
        Tags tags,
        ConfigDefinition config,
        CapabilityBinding<T> binding,
        List<CapabilityExposure> exposures
) {
}
```

该模型可以保存无副作用 Factory 或已解析 Binding，但不能保存 Provider 实例、配置值和运行状态。所有集合
和 Map 必须防御性复制。

### 5.3 PluginDefinitionSnapshot

Snapshot 是持久化专用摘要，包含身份、版本、来源、CapabilityKey、providerId、bind/exposure 摘要和
Contribution 资源身份。Snapshot 不得通过直接序列化 ResolvedPluginDefinition 生成。
...
7. plugin.start
8. stage Contribution、Capability 和 exposure
9. commit staged handles，但仍由 availability 隐藏
...
4. 从活动索引撤出 exposure、Contribution 和 Capability
5. 逆序销毁 Provider
6. plugin.stop
7. close PreparedContribution 和 ResourceScope
8. 清除强引用
9. STOPPED
```
