...
Plugin 模块提供运行时 Handler 与内存活动目录；Console 模块负责将 ACTIVE 贡献同步为持久化索引
（`innospots-nexus-console` → `console.catalog.*` → `nx_console_catalog_resource`）。二者分工如下。
...
宿主启动后，`ConsoleCatalogSyncService`（console 模块）把 ACTIVE 插件贡献与 Page DSL 同步到
