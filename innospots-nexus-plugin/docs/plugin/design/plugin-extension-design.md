...
提供异常、状态码、国际化对象、资源 SPI 等无中间件基础契约，不包含插件发现、安装和运行状态。
...
- 持久化基类与审计填充；
- Quartz 调度、服务节点注册、watcher、启动 SPI；
- 文件元数据（`nx_meta_resource`）与 `ResourceStorageRegistry`。

Core 不包含插件运行时，也不定义 console catalog 索引表。

### 5.3 Plugin

Plugin 模块（artifact `innospots-nexus-plugin`，包 `core.plugin.*`）拥有：
...
- `console@1` 声明模型、Decoder/Handler/Snapshotter、活动内存目录 `ConsoleContributionCatalog`；
- Pactor Page DSL 1.0。

Plugin 不绑定 Spring Boot 自动配置，不持久化 `nx_console_catalog_resource`。

### 5.4 Console

Console 拥有管理平台扩展面：

- 插件管理 Endpoint 和组合视图（VO/Converter）；
- **持久化 catalog 索引**（`console.catalog.*`、`nx_console_catalog_resource`、`ConsoleCatalogSyncService`）；
- Jakarta REST 契约与 workspace 级授权读模型。
