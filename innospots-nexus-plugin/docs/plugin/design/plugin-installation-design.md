...
基础设施，位于 `innospots-nexus-plugin`（依赖 `innospots-nexus-core` 持久化基类）。Console 只提供管理接口和展示模型。
...
当前仓库按绿地库路径直接创建 `nx_plugin_installation`，参考 DDL 位于
`docs/plugin-installation-schema.sql`。本次实现不自动迁移或猜测旧表数据；部署已有数据库时，必须先由人工审核
并提供显式映射：
...
desiredEnabled=enabled；旧 ACTIVE/FAILED 只进入 lastRuntimeState 作为诊断。映射缺失时禁止迁移，旧
`nexus_extension_installation` 的物理删除由部署迁移脚本按数据核对结果执行，Core 代码不再映射该表。
