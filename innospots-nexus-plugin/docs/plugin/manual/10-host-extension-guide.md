...
> **模块边界**：`console@1` 声明模型、Decoder/Handler/Snapshotter、活动内存目录（`ConsoleContributionCatalog`）在 **`innospots-nexus-plugin`**（`core.plugin.contribution.console`）。持久化 catalog 索引（`nx_console_catalog_resource`、`ConsoleCatalogSyncService`）在 **`innospots-nexus-console`**（`console.catalog.*`）。管理 REST（如 `PluginManagementEndpoint`）及 VO/Converter 也在 console。
