...
```xml
<dependencies>
    <!-- Nexus 平台：core 持久化基类 + plugin 运行时 -->
    <dependency>
    ...
    <dependency>
        <groupId>com.innospots.nexus</groupId>
        <artifactId>innospots-nexus-plugin</artifactId>
    </dependency>
    <dependency>
        <groupId>com.innospots.nexus</groupId>
...
**原则**：Core / Plugin / Console 模块本身不依赖 Quarkus；所有 Quarkus 适配代码放在**应用模块**。
Quarkus 索引依赖需同时声明 `innospots-nexus-core` 与 `innospots-nexus-plugin`（见 `quarkus.index-dependency`）。
