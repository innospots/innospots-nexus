...
Base 不依赖 Console、Portal、Servlet、Spring 或数据库，也不包含角色和用户组授权。

### 4.4 Portal 权限业务

`innospots-nexus-portal` 从 `ConsoleContributionCatalog` 读取已激活的管理平台资源，负责：
...
Portal 不依赖插件实现类、插件 DAO 或插件安装表；插件运行时和安装记录由 Core 提供。

### 4.5 应用适配层

具体运行时负责一次性装配：

- 向 Portal 发现器提供用于 SPI 加载的 ClassLoader；
...
Portal 和 Console 不绑定 Spring Boot 自动配置。
...
因此 Portal 的权限实现与实际拦截器适配彼此解耦。后续可以分别提供 Servlet `Filter`、Jakarta REST
...
包。应用运行时如需 Servlet Filter 或 Jakarta REST 适配器，应在应用适配层读取实际请求并调用 Portal
提供的框架无关拦截契约。

Portal 仍以 `permission` 为业务域根包，再按职责组织：

```text
com.innospots.nexus.portal.permission
  endpoint
  dao
  domain
    entity
