...
/**
 * 模拟 Chat 大模型增量输出的 MVC 流式夹具。
 */
@RestController
@Profile("!webflux")
public class SampleChatStreamResource {

    private static final String[] ASSISTANT_TOKENS = {"你", "好", "，", "我是", " Nexus", " 助手"};

    private final StreamManager streamManager;

    public SampleChatStreamResource(StreamManager streamManager) {
        this.streamManager = streamManager;
    }

    /**
     * 打开 {@link StreamSession}，在后台按 token 推送 {@code message.delta}，最后 {@code message} + complete。
     *
     * @return 流式会话
     */
    ...
    public StreamSession<ChatStreamEvent> streamChat() {
        ...
        Thread worker = new Thread(() -> pumpAssistantReply(session), "fixture-chat-stream");
